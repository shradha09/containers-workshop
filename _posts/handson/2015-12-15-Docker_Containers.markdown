...
_Note_: Make sure the Docker daemon is running on the host and you can connect to the docker client. You will also need some containers in a stopped or running state to delete them.
...
_Note_: The Docker daemon will remove the read/write layer, which was created while starting the container.
